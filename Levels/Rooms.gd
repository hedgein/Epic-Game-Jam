...
const gear_coin : PackedScene = preload("res://Item.tscn")
...
func get_room_data(type: int) -> Dictionary:
	...
	# gets all the coin tiles placed in the tilemap and replaces it with a coin scene
	for coin in room.get_used_cells_by_id(room.tile_set.find_tile_by_name("CoinSpawn")):
		var coin_instance = gear_coin.instance()
		coin_instance.position = room.map_to_world(coin) + room.cell_size / 2
		room.add_child(coin_instance)
